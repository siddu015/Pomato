from pomato.commands.cart import display
from pomato.services.cart import CartService
from pomato.models import Cart, Order
...
class OrderService:
    ...
    def checkout(self, user, amount):
        ...
        if orders.count() == 0:
            raise PomatoExit("Place your order before checking out!")
        ...
        if amount < total_price:
            raise PomatoExit("Insufficient balance")
    ...
    def history(self, user):
        ...
        if orders.count() == 0:
            raise PomatoExit("You have not bought anythin yet.")
